...
사실 `useState`가 상태를 유지하는 방식도 같은 원리로 이해할 수 있다. 함수 컴포넌트는 렌더마다 실행되고 끝나는데 상태가 살아남는 이유는, React가 상태를 컴포넌트 함수 바깥에 가둬두고 훅이 그걸 참조하기 때문이다(실제 구현은 fiber라는 자료구조에 저장하지만, 구조는 카운터 예제와 같다 — 상태를 밖에 가두고 접근 통로만 내어준다). 클로저를 모르면 React의 절반은 왜 이렇게 동작하는지  이해하기 힘들 수 있다.
