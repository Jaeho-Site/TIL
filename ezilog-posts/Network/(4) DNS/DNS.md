...
가장 단순하고 직관적인 방식이다. 브라우저는 이 값을 받아 바로 서버로 연결을 시도한다.
...
이 경우 브라우저는 한 번 더 DNS 조회를 통해최종 IP를 알아낸다.

👉 "CDN이나 외부 서비스에 연결할 때 필수"
...
이 문제를 해결하기 위해 등장한 것이 ALIAS 레코드다. ALIAS는 내부적으로는 CNAME처럼 동작하지만,외부에는 최종 IP(A 레코드 형태)를 반환한다.

👉 "루트 도메인을 CDN에 연결해야 할 때"

**왜 이게 중요할까?**

현대 웹 서비스는 하나의 서버에 고정된 구조가 아니다.

- CDN
- 로드 밸런서
- 클라우드 인프라

이 모든 시스템은 IP가 계속 바뀔 수 있다. 그래서 단순한 A 레코드만으로는 부족하고, CNAME이나 ALIAS를 통해 **유연하게 연결하는 구조**가 필요해진다.
